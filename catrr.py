...
def generateData(tNum=180):
    ...
    for l in aoi_file.readlines():
        ...
        loc[0]+=float(items[1])
        loc[1]+=float(items[2])
        spots.append(Spot(spotId=int(items[0]), location=(float(items[1]), float(items[2])), cMax=2*spotStatistic[0][0][int(items[0])], rMax=spotStatistic[0][1][int(items[0])], timeCost=0.4+random.random()/10.0, alpha=1.2, num=spotStatistic[0][0][int(items[0])]))
    loc[0]/=len(spots)
    loc[1]/=len(spots)
    tourists=[]
    for i in range(len(tourists), tNum):
        ...
        tourists.append(Tourist(touristId=i, timer=Timer_v6(elapsed=2+2*random.random()), startSpot=s, endSpot=e, speed=random.uniform(5, 10), loc=loc))
...
class RREnv(Env):
    def __init__(self, spots=0, tourists=0):
        # if spots==0 and tourists==0:
        #     spots, tourists=generateData()
        # elif spots==0 and tourists!=0:
        #     spots, _=generateData()
        # if spots!=0 and tourists==0:
        #     _, tourists=generateData()
        # else:
        #     print("Accepted the given spots&tourists!")
        # 可能的动作：什么也不干、去第n个景点，
        ...
        # self.log = open("visual/dynamicReward_"+timestamp+"_spotStatistic.log", "a")
        self.log = None
    ...
    def step(self, action):
        ...
        # 执行动作
        # ====================================================================
        avail, reward, commuteTime =self.calculateReward(touristId, action)
        if avail:
            # 如果能执行，就执行动作
        ...
        if nextTourist != -1:
            ...
            for spot in self.spots:
                ...
                # 如果去了这个景点之后还能去终点
                if able:
                    avail, reward, commuteTime =self.calculateReward(nextTourist, spot.spotId)
