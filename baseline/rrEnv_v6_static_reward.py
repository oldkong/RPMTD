...
class Tourist:
    ...
    def act(self, env, action, reward, commuteTime):
        ...
        print(self.touristId, "+=1 ", action, " : ", env.spots[action].num)
        # 累加游客游览景点获得的回报
        # self.reward+=reward
    ...
    def __str__(self):
        return "Tourist {}: startTime: {}, start_point: {}, startSpot: {}, endSpot: {},\n realRoute: {},\n Reward: {},\n staticReward: {}, elapsed: {}, nextFinish: {}, complete: {}.\n".format(self.touristId, self.timer.startTime, self.start_point, self.startSpot, self.endSpot, self.realRoute, self.reward, self.staticReward, self.timer.getElapsed(), self.timer.nextFinish, self.complete) 

spotStatistic=[[[], []] for i in range(7)]
def generateData(numt=100):
...
def generateDataSim(numt=200):
    ...
    skey=sorted(sSim)
    ekey=sorted(eSim)
    ss=[sSim[skey[0]], sSim[skey[1]], sSim[skey[2]]]
    es=[eSim[ekey[0]], eSim[ekey[1]], eSim[ekey[2]]]
...
class RREnv(Env):
    def __init__(self):
        ...
        spots, tourists=generateDataSim()
        # 可能的动作：什么也不干、去第n个景点，
            ...
            if self.checkAvailabilityAct(touristId, action):
                ...
                if self.checkCompleteAvailability(touristId, action):
                    ...
                    if len(self.tourists[touristId].route) == 0:
                        if action == self.tourists[touristId].startSpot:
                            ...
                            print(touristId, " : ", reward, " 1.1")
                        else:
                            reward = -2*reward if reward>=0 else 2*reward
                            print(touristId, " : ", reward, " 1.2")
                    # 如果已经开始游览
                    else:
                        # 如果去过这个景点了
                        ...
                        # 还没去过
                            ...
                            # 不是终点
                                ...
                                # 不是其他游客的起点或终点
                                    print(touristId, " : ", reward, " 2.2")
                                else:
                                    reward = reward if reward>=0 else reward
                                    print(touristId, " : ", reward, " 2.3")
                    # print("Y"*100)
                ...
                # 如果去了这个景点就不能去终点了
                    ...
                    if action == self.tourists[touristId].endSpot:
                        if len(self.tourists[touristId].route)!=0: 
                            reward = 2*reward if reward>=0 else -2*reward
                            print(touristId, " : ", reward, " 3.1")
                            return True, reward, commuteTime
                        else:
                            reward = -2*reward if reward>=0 else 2*reward
                            print(touristId, " : ", reward, " 3.2")
                            return True, reward, commuteTime
                    else:
                        reward = -2*reward if reward>=0 else 2*reward
                        return False, reward, commuteTime
    ...
    def step(self, action):
        ...
        if avail:
            ...
            if result and len(self.tourists[touristId].route)>1:
                ...
                print(touristId, "1 -=1 ", self.tourists[touristId].route[-2], " : ", self.spots[self.tourists[touristId].route[-2]].num)
        ...
        if self.tourists[touristId].endSpot in self.tourists[touristId].route:
            ...
            if len(self.tourists[touristId].route)>0:
                ...
                print(touristId, "2 -=1 ", self.tourists[touristId].route[-1], " : ", self.spots[self.tourists[touristId].route[-1]].num)
        else:
            if self.tourists[touristId].timer.getElapsed()>0:
                ...
                if len(self.tourists[touristId].route)>0:
                    self.spots[self.tourists[touristId].route[-1]].num-=1            
                    print(touristId, "3 -=1 ", self.tourists[touristId].route[-1], " : ", self.spots[self.tourists[touristId].route[-1]].num)
                done=True
        # ====================================================================
        # 选择下一个执行动作的tourist
        # ====================================================================
        ...
        #     print(t.touristId," : ", t.timer.nextFinish)
        for tourist in self.tourists:
        ...
        return r_overtime, sReward, done, nextTourist
